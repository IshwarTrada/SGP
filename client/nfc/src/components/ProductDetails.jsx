...
const ProductDetail = () => {
    ...
    const handleAddToCart = () => {
        ...
        console.log(`Added to cart: ${count} x ${product.name}, Total Price: Rs.${totalPrice}`);
    };
                ...
                <div className="flex flex-col flex-1 gap-6">
                        ...
                        <div className="flex items-center gap-1">
                            <div className="flex gap-1">
                                {[...Array(fullStars)].map((_, index) => (
                                    <i key={index} className="bx bxs-star text-[#fa8232] text-lg" />
                                ))}
                                {halfStar && <i className="bx bxs-star-half text-[#fa8232] text-lg" />}
                                {[...Array(5 - fullStars - (halfStar ? 1 : 0))].map((_, index) => (
                                    <i key={index + fullStars + (halfStar ? 1 : 0)} className="bx bx-star text-gray-300 text-lg" />
                                ))}
                            </div>
                            <span className="text-lg font-semibold">{product.rating.toFixed(1)} Star Rating</span>
                            <span className="text-gray-600">({product.userFeedbackCount} User feedback)</span>
                        </div>
                        <h3 className="text-3xl font-normal">{product.name}</h3>
                    </div>
                    <div className="flex flex-col gap-1 text-gray-600">
                        <div className="flex gap-10">
                            ...
                            <div className="flex flex-col">
                                <span>Availability : <span className="font-semibold text-[#4aaa4f]">{product.availability}</span></span>
                            </div>
                        </div>
                    </div>
                    <div className="flex flex-row items-start gap-2 text-secondary-500">
                        <div className="text-2xl font-semibold">Rs.{product.price}</div>
                        <div className="flex flex-col">
                            <div className="line-through text-gray-500 text-xl font-semibold mt-[1px]">Rs.{product.originalPrice}</div>
                        </div>
                        <div className="flex flex-col">
                    ...
                    <div className="flex flex-row items-start gap-4">
                        <div className="flex flex-col gap-4 ml-0">
                            <div className="line-height-20">Have a design in mind? Share it here...</div>
                            <div className="flex flex-row gap-8 ">
                                ...
                                >
                                    <label htmlFor="attach-file" className="cursor-pointer flex-1">
                                        {file ? file.name : 'Drag or Drop your design'}
                                    </label>
                                    <input id="attach-file" type="file" className="hidden" onChange={handleFileChange} />
                                    <div className="flex flex-col">
                                        <img src={product.uploadImageIcon} alt="Upload Icon" className="w-4 h-6 ml-4" />
                                    </div>
                                </div>
                                <div className="flex justify-end">
                                    <button className="border border-gray-500 bg-transparent px-4 py-1 rounded-sm cursor-pointer" onClick={() => console.log(file)}>
                            ...
                            <div className="flex items-start justify-end gap-2">
                                ...
                                <button className="flex items-center justify-center bg-gray-500 text-white uppercase px-4 h-[52px] w-[200px] py-2 rounded-sm" onClick={handleAddToCart}>
                                    <b>Add to cart</b>
                    ...
                    <div className="flex justify-between items-center text-gray-700">
                        <div className="flex items-center gap-2">
                            ...
                            >
                                <i className={`bx bx-heart ${isWished ? 'bxs-heart' : 'bx-heart'}`} />
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <a href="http://www.twitter.com">
                                    <img src="./src/assets/Twitter.svg" alt="Twitter" className="w-4 h-4" />
                                </a>
                                <a href="http://www.pinterest.com">
                                    <img src="./src/assets/Pinterest.svg" alt="Pinterest" className="w-4 h-4" />
                                </a>
                            </div>
                        </div>
                    </div>
                    <div className="flex flex-col rounded-sm border border-gray-100 p-8 shadow-lg">
                        ...
                        <div className="flex gap-2 pt-4">
                            {product.paymentMethods.map((method, index) => (
                                <img key={index} src={method} alt="Payment Method" className="max-w-[80px] max-h-[80px]" />
                            ))}
                        </div>
                    </div>
                </div>
